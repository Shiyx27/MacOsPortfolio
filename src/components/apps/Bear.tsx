    ...
    <ul>
      {items.map((item: BearMdData, index: number) => (
        <li
          ...
          className={`flex flex-col cursor-default border-l-2 mb-4 py-2 ${
            cur === index
        ...
        >
          <div className="mt-2 hstack">
            ...
            <span className="relative flex-1 font-bold" text="gray-900 dark:gray-100">
              {item.title}
              <div pos="absolute top-1 right-3 hstack space-x-2">
                {item.link && (
                  <a
                    href={item.link}
                    target="_blank"
                    rel="noreferrer"
                    title="Open app"
                    className="inline-flex items-center justify-center w-7 h-7 rounded-full p-0.5 hover:bg-gray-100 dark:hover:bg-gray-800"
                  >
                    <span className="i-ant-design:link-outlined text-c-500 text-sm" />
                  </a>
                )}
                {item.repo && (
                  <a
                    ...
                    className="inline-flex items-center justify-center w-7 h-7 rounded-full p-0.5 hover:bg-gray-100 dark:hover:bg-gray-800"
                  >
                    <span className="i-icon-park-outline:github text-c-500 text-sm" />
                  </a>
                )}
              </div>
            </span>
          </div>
          <div className="flex-1 ml-10" p="b-3 r-1" text="sm c-500" border="b c-300">
            {item.excerpt}
          </div>
        </li>
      ))}
